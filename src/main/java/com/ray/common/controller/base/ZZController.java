...
//	}
	...
	public void getSonData() {
		try {
			...
			List<DataField> fields = DataField.dao.find("select * from data_field where data_object_id = "+menu.getSonDataObjectId());
			String sqlString = "select ";
			for (int i = 0; i < fields.size(); i++) {
				if(i>0) {
					sqlString += ",";
			...
			sqlString += " from "+model.getTableName()+" t where "+menu.getSonIdField()+" = "+row.getIntValue(menu.getParentIdField())+" ";
			sqlString += model.getWhereAttr()!=null?model.getWhereAttr():"";
