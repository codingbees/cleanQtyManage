...
public class SingleController extends BaseController {
	 ...
	 */
//	public void single(){
//		set("object_id", get("object_id"));
//		set("parent_id", get("parent_id"));
//		set("parent_id_field", get("parent_id_field"));
//		set("user_roles",(String)getSessionAttr("user_roles"));
//		if(!"null".equals(get("url")) && !"".equals(get("url"))) {
//			render(get("url"));
//		}
//	}
	...
	public void getColumns() {
		...
		//获取头部buttons
		List<DataButton> headButtons = DataButton.dao.find("select * from data_button where data_object_id = "+get("object_id")+" and location=1 order by order_num");
		resp.set("headButtons", headButtons);
		//获取行内buttons
		List<DataButton> lineButtons = DataButton.dao.find("select * from data_button where data_object_id = "+get("object_id")+" and location=2 order by order_num");
	...
	public void getData(){
		...
		if(query!=null) {
			for (int i = 0; i < fields.size(); i++) {
				if(fields.get(i).getIsQuery() && query.get(fields.get(i).getEn())!=null) {
					if(fields.get(i).getIsFictitious()) {
						sqlString += " and ("+fields.get(i).getFictitiousSql()+") like '%"+query.get(fields.get(i).getEn())+"%'";
		...
		if(get("parent_id")!=null && !"".equals(get("parent_id"))) {
			sqlString += " and "+get("parent_id_field")+" = "+get("parent_id");
		}
	...
	public void new_() {
		try {
			...
			//是否有父节点
			if(get("parent_id")!=null && !"".equals(get("parent_id"))) {
				model.set(get("parent_id_field"), get("parent_id"));
			}
			//处理附件
				...
				for (int i = 0; i < fileArray.length; i++) {
					...
					if(!"".equals(jb.getString("response")) && jb.getString("response")!=null) {
						...
						try {
							...
							e.printStackTrace();
							renderJson(Ret.fail("msg", "新增失败，原因："+e.getMessage()));
							return;
						}
					}
				}
		...
		try {
			...
			List<Record> list = Db.use("sys")
					.find("select * from columns where table_schema = '"+PropKit.get("table_schema")+"' and table_name = '"
							+ object.getTableName() + "' order by ORDINAL_POSITION");
			for (int i = 0; i < list.size(); i++) {
