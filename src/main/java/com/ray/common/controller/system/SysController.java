...
public class SysController extends BaseController {
	...
	public void getUser() {
		...
		for (int i = 0; i < userList.size(); i++) {
			...
			List<Record> roleIds = Db
					.find("select role_id from user_role where user_id = " + ((Record) userList.get(i)).get("id"));
	...
	public void userform() {
		...
		if (userId != 0) {
			...
			setAttr("user", user);
			Record roles = Db
					.findFirst("SELECT GROUP_CONCAT(role_id) role_ids FROM user_role WHERE user_id = '" + userId + "'");
	...
	public void getMenuTree() {
		...
		top_menu = menuAuth(top_menu, getParaToInt(0).intValue());
		trees.set("trees", getMenus(top_menu, getParaToInt(0).intValue()));
		rsp.set("data", trees);
		renderJson(rsp);
	}
	 ...
	 */
	@NotAction
	public List<Record> getMenus(List<Record> menus, int roleId) {
		for (int i = 0; i < menus.size(); i++) {
			List<Record> children_menu = Db.find("select * from menu where parent_menu = " + menus.get(i).getInt("id")
					+ " and is_hide = 0 order by seq_num");
			if (children_menu.size() > 0) {
				children_menu = getMenus(menuAuth(children_menu, roleId), roleId);
			}
			menus.get(i).set("list", menuAuth(children_menu, roleId));
		}
		return menus;
	}
	...
	public void clearMenuAuth(int roleId) {
		List<Record> permissions = Db.find(
				"SELECT * FROM permissions WHERE TYPE = 1 AND id IN (SELECT permission_id FROM role_permission WHERE role_id = "
						+ roleId + ")");
		for (int i = 0; i < permissions.size(); i++) {
			int perId = ((Integer) ((Record) permissions.get(i)).get("id")).intValue();
			Db.deleteById("permissions", Integer.valueOf(perId));
			Db.delete("delete from role_permission where role_id = ? and permission_id = ?",
					new Object[] { Integer.valueOf(roleId), Integer.valueOf(perId) });
		}
	}
	...
	public List<Record> menuAuth(List<Record> menu, int roleId) {
		String sql = "";
		if (roleId != 0) {
			sql = "SELECT gl_id FROM permissions WHERE id IN (SELECT permission_id FROM role_permission WHERE role_id = "
					+ roleId + ") AND TYPE = 1";
		} else {
