...
protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals)
  {
    ...
    String loginName = (String)principals.fromRealm(getName()).iterator().next();
      ...
      for (int i = 0; i < roleList.size(); i++) {
        ...
        List<Record> permissionList = Db.find("select * from role_permission where role_id = ?", new Object[] { ((Record)roleList.get(i)).getInt("role_id") });
        for (int j = 0; j < permissionList.size(); j++) {
          Record permission = Db.findById("permissions", ((Record)permissionList.get(j)).getInt("permission_id"));
