...
var Methods = {
  ...
  cacheColumnMap: function cacheColumnMap() {
    ...
    if (isGroup) {
      xe_utils_amd_xe_utils_default.a.eachTree(collectColumn, handleFunc);
    } else {
...
function renderDateMonthTable(h, _vm) {
  ...
  return [h('table', {
    ...
    return h('tr', rows.map(function (item) {
      ...
      }, conf.i18n("vxe.input.date.months.m".concat(item.month)));
    }));
  }))])];
}
...
function renderPasswordIcon(h, _vm) {
  ...
  return h('span', {
    ...
    class: ['vxe-input--pwd-icon', showPwd ? conf.icon.INPUT_SHOW_PWD : conf.icon.INPUT_PWD]
  })]);
}
    ...
    handleResize: function handleResize() {
      ...
      if (this.autosize) {
        this.$nextTick(function () {
          ...
          var textRows = Math.floor((sizeHeight - intervalHeight) / lineHeight) + 1;
          ...
          this.$nextTick(function () {
            ...
            if (!remember || !inited) {
              ...
              if (left && !leftCenter) {
                ...
                posLeft = "".concat(clientVisibleWidth / 2 - modalBoxElem.offsetWidth / 2, "px");
              }

              if (topCenter) {
                posTop = "".concat(clientVisibleHeight / 2 - modalBoxElem.offsetHeight / 2, "px");
